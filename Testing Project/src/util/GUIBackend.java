...
//			pane.getStyleClass().add("pane");
	...
	public static void displayUserPosts(LinkedList<UUID> userPostsUUIDs, TilePane tilePane, LandingController landingController) {
		Iterator<UUID> itr = userPostsUUIDs.descendingIterator();
		while(itr.hasNext()) {
			try {
				FXMLLoader fxmlLoader = new FXMLLoader();
				fxmlLoader.setLocation(GUIBackend.class.getResource("/views/Post.fxml"));
				AnchorPane ap = fxmlLoader.load();
				PostController postController = fxmlLoader.getController();
				postController.setPostData(PostCenter.getInstance().getPost(itr.next()));
				postController.setLandingController(landingController);
				tilePane.getChildren().add(ap);	
			} catch (IOException e) {
				e.printStackTrace();
